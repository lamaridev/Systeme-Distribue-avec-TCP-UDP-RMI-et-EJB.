...
public class client5 {
	...
	public static void main(String[] args) {
		...
		String[] reference5 = {"S2", "S14", "S1", "S7", "S6", "S3", "S0", "S5", "S2", "S15", "S7", "S0", "S4", "S12", "S3", "S6", "S7", "S2", "S1", "S13","FIN"};
	    i=0;
		...
		try {
		  ...
		  System.out.println("client 5 attend le jeton ");
	      t1.startTimer(8000);
	      System.out.println("Starting Timer....");


		  DatagramSocket s1 = new DatagramSocket(1005);
		  ...
		  {jeton=Integer.parseInt(receivedMessage); System.out.println("Bonne reception du jeton ..");		        LocalTime heureActuelle = LocalTime.now();
	        ...
	        String formattedTime = heureActuelle.format(formatter); System.out.println("Reçu a : " + formattedTime);}
			    ...
			    System.out.println("j'ai pas reçu le jeton du client 4 dans le délai ");
	  		...
	  		System.out.println("Demande de service en cours...");
				...
				if (i<reference5.length && jeton==1)
				{	if(reference5[i].equals("FIN")) {
					t1.stopTimer();
					...
					DatagramPacket p = new DatagramPacket(Mdata, Mdata.length,ip,1006);
					dg.send(p);
					dg.close();
					jeton=0;
