...
public class client3 {
	...
	public static void main(String[] args) {
		...
		String[] reference3 = {"S10", "S3", "S6", "S4", "S2", "S7", "S0", "S3", "S12", "S8", "S7", "S0", "S6", "S5", "S3", "S2", "S1", "S7", "S0", "S4", "FIN"};
	    i=0;
		...
		try {
	      ...
	      System.out.println("client 3 attend le jeton ");
	      t1.startTimer(100000);
	      System.out.println("Starting Timer....");


		  DatagramSocket s1 = new DatagramSocket(1003);
		  ...
		  {jeton=Integer.parseInt(receivedMessage); System.out.println("Bonne reception du jeton ..");		        LocalTime heureActuelle = LocalTime.now();
	        ...
	        String formattedTime = heureActuelle.format(formatter); System.out.println("Reçu a : " + formattedTime);}
			    ...
			    System.out.println("j'ai pas reçu le jeton du client 2 dans le délai ");
	  		...
	  		System.out.println("Demande de service en cours...");
				...
				if (i<reference3.length && jeton==1)
				{	if(reference3[i].equals("FIN")) {
					t1.stopTimer();
					...
					DatagramPacket p = new DatagramPacket(Mdata, Mdata.length,ip,1004);
					dg.send(p);
					dg.close();
					jeton=0;
