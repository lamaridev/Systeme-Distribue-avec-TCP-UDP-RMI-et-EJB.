...
public class client6 {
	...
	public static void main(String[] args) {
		...
		String[] reference6 = {"S5", "S4", "S1", "S3", "S10", "S7", "S6", "S0", "S13", "S2", "S1", "S7", "S6", "S3", "S15", "S2", "S1", "S14", "S5", "S6","FIN"};
	    i=0;
		...
		try {
		  ...
		  System.out.println("client 6 attend le jeton ");
		  t1.startTimer(100000);
		  System.out.println("Starting Timer....");
			  
		  DatagramSocket s1 = new DatagramSocket(1006);
		  ...
		  {jeton=Integer.parseInt(receivedMessage); System.out.println("Bonne reception du jeton ..");		        LocalTime heureActuelle = LocalTime.now();
	        ...
	        String formattedTime = heureActuelle.format(formatter); System.out.println("Reçu a : " + formattedTime);}
			    ...
			    System.out.println("j'ai pas reçu le jeton du client 5 dans le délai ");
	  		...
	  		System.out.println("Demande de service en cours...");
				...
				if (i<reference6.length && jeton==1)
				{	if(reference6[i].equals("FIN")) {
					t1.stopTimer();
					...
					DatagramPacket p = new DatagramPacket(Mdata, Mdata.length,ip,1001);
					dg.send(p);
					dg.close();
					jeton=0;
