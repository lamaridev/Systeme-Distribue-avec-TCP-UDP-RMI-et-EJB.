...
public class client4 {
	...
	public static void main(String[] args) {
		...
		String[] reference4 = {"S13", "S2", "S1", "S7", "S0", "S4", "S6", "S5", "S11", "S0", "S14", "S3", "S9", "S1", "S0", "S3", "S6", "S7", "S12", "S11", "FIN"};
	    i=0;
		...
		try {
		  ...
		  System.out.println("client 4 attend le jeton ");
	      t1.startTimer(6000);
	      System.out.println("Starting Timer....");


		  DatagramSocket s1 = new DatagramSocket(1004);
		  ...
		  {jeton=Integer.parseInt(receivedMessage); System.out.println("Bonne reception du jeton ..");		        LocalTime heureActuelle = LocalTime.now();
	        ...
	        String formattedTime = heureActuelle.format(formatter); System.out.println("Reçu a : " + formattedTime);}
			    ...
			    System.out.println("j'ai pas reçu le jeton du client 3 dans le délai ");
	  		...
	  		System.out.println("Demande de service en cours...");
				...
				if (i<reference4.length && jeton==1)
				{	if(reference4[i].equals("FIN")) {
					t1.stopTimer();
					...
					DatagramPacket p = new DatagramPacket(Mdata, Mdata.length,ip,1005);
					dg.send(p);
					dg.close();
					jeton=0;
